...
On using authenticated signature, the user's identity is verified by ID-porten. ID-porten does not know that the login is connected to a signing ceremony and it is Signicat's job to link the verification of identity and the documents to a signature.
